...
st.set_page_config(
    page_title="Macdelorean Radar v27",
    page_icon="🚗",
    layout="wide"
)
...
with st.sidebar:
    ...
    <div style='text-align:center; padding: 4px 0 10px 0; border-bottom: 1px solid #2A1E08;'>
        ...
        <div style='font-family: Share Tech Mono, monospace; font-size: 0.6rem; color: #6B5010; letter-spacing: 3px; margin-top:2px;'>RADAR v27</div>
    </div>
    """, unsafe_allow_html=True)
...
if lanzar:
            ...
            for tf_key, tf_name, est_sel, cer_sel in tfs_velas:
                for j in range(4):
                    ...
                    if es_patron:
                        ...
                        if not ((es_alc and dir_alcista) or (not es_alc and dir_bajista)):
                            continue  # v27: mirar velas más antiguas en la dirección pedida
    ...
    # v27: guardar resultados en sesión → pulsar "Exportar CSV" ya no borra el escaneo
    st.session_state['radar'] = {
        'hora':         time.strftime('%d/%m/%Y %H:%M'),
        'n_escaneados': len(master_list),
        'fallos':       fallos,
        'res':   {n: globals()[f'res_{n}'] for n in ['prem', 'velas', 'diverg', 'macd', 'confluencia', 'emas', 'puntob', 'paco', 'macdelorean', 'sabroson200', 'sabroson50', 'conf_master']},
        'flags': {f: globals()[f'filtro_{f}'] for f in ['premium', 'velas', 'diverg', 'macd_combo', 'confluencia', 'emas', 'puntob', 'paco', 'macdelorean', 'sabroson200', 'sabroson50', 'conf_master']},
    }

if st.session_state.get('radar'):
    _R = st.session_state['radar']
    for _n, _v in _R['res'].items():
        globals()[f'res_{_n}'] = _v
    for _f, _v in _R['flags'].items():
        globals()[f'filtro_{_f}'] = _v

    st.caption(f"📅 Resultados del escaneo del {_R['hora']} · {_R['n_escaneados']} tickers")
    if _R['fallos']:
        with st.expander(f"⚠️ {len(_R['fallos'])} tickers sin datos (delistados o bloqueo de Yahoo)"):
            st.write(", ".join(_R['fallos']))

    st.markdown("---")
    _metricas = [
        ("🎯 Escaneados",   _R['n_escaneados']),
        ("💎 Premium",      len(res_prem)),
        ("🕯️ Velas Cambio", len(res_velas)),
        ("📐 Divergencias", len(res_diverg)),
        ("📡 MACD Combo",   len(res_macd)),
        ("💥 Confluencia",  len(res_confluencia)),
        ("📈 EMA Cross",    len(res_emas)),
        ("🔵 Punto B",      len(res_puntob)),
        ("🌟 Paco Pérez",   len(res_paco)),
        ("🚗 Macdelorean",  len(res_macdelorean)),
        ("🌮 Sabrosón",     len(res_sabroson200) + len(res_sabroson50)),
        ("💎 Conf Master",  len(res_conf_master)),
    ]
    for _col, (_lab, _val) in zip(list(st.columns(6)) + list(st.columns(6)), _metricas):
        _col.metric(_lab, _val)
    st.markdown("---")
    ...
    if filtro_velas:
        with tabs[tab_idx]:
            if res_velas:
                ...
                st.info("Sin velas de cambio detectadas.")
        tab_idx += 1
