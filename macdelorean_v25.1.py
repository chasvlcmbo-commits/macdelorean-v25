...
def check_divergencia(df, timeframe="D"):
    ...
    if len(mins_validos) >= 2:
        ...
        # El último swing debe estar cerca del presente (no más de 8 velas atrás)
        # y el precio actual no puede estar haciendo nuevos mínimos por debajo del swing
        max_antiguedad = {"D": 10, "W": 8, "M": 6}.get(timeframe, 8)
        if (total - 1 - p2) > max_antiguedad:
            pass  # último swing demasiado antiguo, no es divergencia activa
        elif df['Low'].iloc[p2:].min() < df['Low'].iloc[p2]:
            pass  # el precio ha hecho nuevos mínimos después del swing — divergencia rota
        elif (p2 - p1) >= min_velas:
    ...
    if len(maxs_validos) >= 2:
        ...
        max_antiguedad = {"D": 10, "W": 8, "M": 6}.get(timeframe, 8)
        if (total - 1 - p2) > max_antiguedad:
            pass  # último swing demasiado antiguo
        elif df['High'].iloc[p2:].max() > df['High'].iloc[p2]:
            pass  # el precio ha hecho nuevos máximos después del swing — divergencia rota
        elif (p2 - p1) >= min_velas:
